...
module.exports = {
   ...
   */
  "ABOR": function () { socket.reply(202) }, // Abort an active file transfer.
  ...
  "PASV": function () { // Enter passive mode
      var port = this.passiveAddress.port
        , host = this.passiveAddress.host
      this.passive = true
      this.reply(227, 'PASV OK (' + host.split('.').join(',') + ',' + parseInt(port/256,10) + ',' + (port%256) + ')')
  },
  /**
   * The PORT command is sent by the client to tell the server what address and port on the client
   * to listen on. The format is 4 bytes for the ip address and two bytes for port, which are
   * passed as a comma separated string of integers.
   */
  "PORT": function (ip1, ip2, ip3, ip4, port1, port2) {
    this.passiveAddress.host = [ip1, ip2, ip3, ip4].join(".").replace(/,/g, '')
    this.passiveAddress.port = parseInt((parseInt(port1) * 256) + parseInt(port2));
