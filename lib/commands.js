...
module.exports = {
  ...
  "PASV": function () { // Enter passive mode
    ...
    dataServer.on('connection', function (dataSocket) {
      ...
      var port = socket.passiveAddress.port
        , host = socket.passiveAddress.host
  ...
  "PORT": function (ip1, ip2, ip3, ip4, port1, port2) {
    this.activeAddress.host = [ip1, ip2, ip3, ip4].join(".").replace(/,/g, '')
    this.activeAddress.port = parseInt((parseInt(port1) * 256) + parseInt(port2));
